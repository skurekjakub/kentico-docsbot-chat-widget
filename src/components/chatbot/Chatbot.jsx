...
export const Chatbot = ({ isOpen, setIsOpen, isEmbeddedBox }) => {
  ...
  useEffect(() => {
    localStorage.setItem(`DocsBot_${botId}_chatHistory`, JSON.stringify(state.messages));
  }, [state.messages]);

  useEffect(() => {
    if (state.chatHistory) {
      localStorage.setItem(`DocsBot_${botId}_localChatHistory`, JSON.stringify(state?.chatHistory));
    }
  }, [state.chatHistory]);
            ...
            if (data.event === 'stream') {
              ...
              if (data.data) {
                ...
                document.dispatchEvent(
                  new CustomEvent('docsbot_fetching_answer_complete', { detail: finalData })
                );
                setIsFetching(false);
              } else {
  ...
  useEffect(() => {
    ...
    root.style.setProperty('--docsbot-image-upload-btn--color', isWhite ? '#314351' : primaryColor);

    // Add drag-and-drop zone styles
    root.style.setProperty('--docsbot-drag-border-color', isWhite ? '#314351' : primaryColor);
  }, [color]);
        ...
        <div className="docsbot-chat-inner-container">
          ...
          >
            <div className="docsbot-chat-header-inner" style={{ width: '100%' }}>
              <button onClick={() => refreshChatHistory()} className="docsbot-chat-header-button">
              ...
              {!isEmbeddedBox && (
                <div className="docsbot-chat-header-background-wrapper">
                  <div className="docsbot-chat-header-background" data-shadow="true" />
                </div>
              )}
            </div>
          </div>
          ...
          <div className="docsbot-chat-message-container" ref={ref}>
            ...
            {Object.keys(state.messages).length <= 1 && Object.keys(questions).length >= 1 && (
              ...
              >
                <span className="docsbot-chat-suggested-questions-title">{labels.suggestions}</span>
          ...
          <div className="docsbot-chat-footer">
            {!isAtBottom && (
              <button
                className={clsx('docsbot-scroll-button', isAtBottom && 'hide')}
              ...
              <div className="docsbot-chat-input-container">
                ...
                >
                  ...
                  <button
                    ...
                    {...(['#ffffff', '#FFFFFF', 'rgb(255, 255, 255)'].includes(color) && {
                      style: { fill: 'inherit' },
                    })}
