...
export const BotChatMessage = ({
  ...
  const isRepeatedBotMessage = () => {
    ...
    const currentIndex = messageIds.findIndex(id => state.messages[id].id === payload.id);
    ...
    try {
      ...
      if (supportCallback && typeof supportCallback === 'function') {
        ...
        if (paramCount <= 2) {
          ...
          await supportCallback(syntheticEvent, history, identify.metadata || {});
        } else {
          ...
          if (payload.conversationId && isAgent) {
            ...
            await supportCallback(syntheticEvent, history, identify.metadata || {}, ticket);
          } else {
            await supportCallback(syntheticEvent, history, identify.metadata || {}, null);
          }
        }
      }
  ...
  const saveRating = async (newRating = 0) => {
    ...
    try {
      ...
      if (isAgent && payload.responses) {
        const ratingMessage = newRating === 1 ? payload.responses.yes : payload.responses.no;
        if (ratingMessage) {
          dispatch({
            type: 'add_message',
            payload: {
  ...
  useEffect(() => {
    if (payload.isLast && supportLink && !isAgent && (payload.sources || payload.error)) {
      scrollToBottom(chatContainerRef);
    }
  ...
  return (
    <>
      ...
      {useFeedback &&
        ...
        !hasNextMessage() && (
          <>
            <div className={clsx('docbot-chat-bot-message-rate', botIcon && 'has-avatar')}>
              <button
                onClick={e => {
                  saveRating(1);
                }}
                title={labels.helpful}
                className={clsx('doscbot-rate-good', rating === 1 && 'selected')}
                {...(rating === 1 && { disabled: true })}
              >
                <span dir="auto" aria-hidden="true">
                  {payload.responses?.yes || labels.feedbackYes || '👍'}
                </span>
              </button>
              ...
              <button
                ...
                className={clsx('doscbot-rate-bad', rating === -1 && 'selected')}
                {...(rating === -1 && { disabled: true })}
              >
                <span dir="auto" aria-hidden="true">
                  {payload.responses?.no || labels.feedbackNo || '👎'}
                </span>
              </button>
            </div>
          </>
      ...
      */}
      {useEscalation &&
        ...
        (supportLink || (supportCallback && typeof supportCallback === 'function')) && (
          <>
            <div className={clsx('docbot-chat-bot-message-rate', botIcon && 'has-avatar')}>
              <button
              ...
              >
                {isSupportLoading ? (
                  ...
                  <span dir="auto" aria-hidden="true">
                    {payload.responses?.yes || labels.getSupport}
                  </span>
                )}
              </button>
